<x-layouts.app
    title="Kinderentertainers.nl — boeken zonder regelwerk"
    meta-description="Vind en boek kinderentertainers voor kinderfeestjes, scholen en events. Filter op act, regio en beschikbaarheid en stuur direct een aanvraag."
    canonical-url="{{ url('/') }}"
>
        ...
        <div class="brand-shell grid gap-10 py-12 lg:min-h-[calc(100vh-9rem)] lg:grid-cols-[1.02fr_0.98fr] lg:items-center lg:py-16">
            <div class="flex flex-col justify-center">
                ...
                <div class="mt-8 flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('register') }}" class="brand-button-secondary">Account aanmaken</a>
                </div>
            ...
            <div class="brand-panel p-3 sm:p-5">
                <div class="rounded-lg bg-brand-ink p-5 text-white">
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <span class="rounded-full bg-white/15 px-3 py-1 text-xs font-black uppercase tracking-[0.18em] text-brand-mint">
                            {{ $entertainerCount > 0 ? 'live match' : 'binnenkort' }}
                        </span>
                    </div>
                    <p class="mt-10 text-xs font-bold uppercase tracking-[0.18em] text-brand-sun">Beschikbaarheid en matching</p>
                    ...
                    <div class="mt-8 grid gap-2 text-sm sm:grid-cols-3">
                        <div class="rounded-md bg-white/10 p-3">
                            <span class="block text-2xl font-black">{{ $entertainerCount }}</span>
                            {{ $entertainerCount === 1 ? 'entertainer' : 'entertainers' }}
                        </div>
                        <div class="rounded-md bg-white/10 p-3">
                            <span class="block text-2xl font-black">{{ $averageRating ? number_format((float) $averageRating, 1, ',', '') : '—' }}</span>
                            reviewscore
                        </div>
                        <div class="rounded-md bg-white/10 p-3">
                            <span class="block text-2xl font-black">4 stappen</span>
                            van aanvraag tot akkoord
                        </div>
                    </div>
                </div>
                <div class="mt-4 grid gap-3 sm:grid-cols-2">
                    <div class="rounded-lg border border-amber-100 bg-amber-50 p-4 shadow-sm">
                        <p class="text-xs font-black uppercase tracking-[0.16em] text-amber-700">Zoeken</p>
                        <p class="mt-2 font-bold text-brand-ink">Skill, regio, datum</p>
                        <p class="mt-1 text-sm text-slate-600">Filter tot een passende act overblijft.</p>
                    </div>
                    <div class="rounded-lg border border-rose-100 bg-rose-50 p-4 shadow-sm">
                        <p class="text-xs font-black uppercase tracking-[0.16em] text-brand-coral">Boeken</p>
                        <p class="mt-2 font-bold text-brand-ink">Offerte en akkoord</p>
                        <p class="mt-1 text-sm text-slate-600">De entertainer factureert zelf.</p>
                    </div>
                </div>
            </div>
        </div>
