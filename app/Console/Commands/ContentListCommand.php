...
class ContentListCommand extends Command
{
    protected $signature = 'content:list
                            {type=all : pages, blog, media of all}';

    protected $description = 'Toon landingspagina\'s, blogposts of foto\'s in de database.';

    public function handle(): int
    {
        $type = strtolower((string) $this->argument('type'));

        if (! in_array($type, ['all', 'pages', 'blog', 'media'], true)) {
            $this->error('Gebruik pages, blog, media of all.');
